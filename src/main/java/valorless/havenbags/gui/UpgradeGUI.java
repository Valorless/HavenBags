...
public class UpgradeGUI implements Listener {
	...
	public void onInventoryClickResult(InventoryClickEvent event) {
		...
		if(resultType == ResultType.Upgrade) {
			...
			Database.getBag(HavenBags.getBagUUID(clicked));
			...
			if (type != null) {
				...
				switch (type) {
					...
					case ItemModel -> {
						NamespacedKey model = NamespacedKey.fromString(value);
						if (BagState.getState(clicked) == BagState.NEW) {
							meta.setItemModel(model);
							clicked.setItemMeta(meta);
						}
						else Database.getBag(HavenBags.getBagUUID(clicked)).setItemmodel(value);
					}
					case ModelData -> {
						...
						else Database.getBag(HavenBags.getBagUUID(clicked)).setModeldata(cmd);
					}
				}
			}
			else {
			...
			Database.getBag(HavenBags.getBagUUID(clicked)).setEffect(value);
